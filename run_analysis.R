...
# selecting features whose name with "mean" or "std", storing thier number into a vector called filter
filter <- c(grep("mean", features$V2), grep("std", features$V2))
filter <- sort(filter)
# extracts variables from data set which matche the filter 
filtered_vars <- paste("V", filter, sep="")
extracted_data <- data[filtered_vars]

# Step 3 - Uses descriptive activity names to name the activities in the data set
# read the training  and test labels into R, and combined them into a data frame which is named 'label'
...
# get mean of each variables in each categorty (actiity*subject)
